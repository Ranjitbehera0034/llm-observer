...
exports.getModelPricing = exports.getAllPricing = exports.refreshPricing = exports.seedPricing = exports.getInitialPricing = void 0;
const db_1 = require("./db");
const pricing_repo_1 = require("./repositories/pricing.repo");
...
const getInitialPricing = () => initialPricing;
exports.getInitialPricing = getInitialPricing;
const seedPricing = () => {
    // Rely on syncPricingToDb to handle wiping stale default data while safely protecting user-injected is_custom overrides.
    (0, pricing_repo_1.syncPricingToDb)(initialPricing);
 ...
 */
const refreshPricing = (pricingData) => {
...
const getModelPricing = (provider, model) => {
    ...
    let pricing = db.prepare('SELECT * FROM model_pricing WHERE provider = ? AND model = ? ORDER BY id DESC LIMIT 1').get(provider, model);
    // If no exact match, try fuzzy match (e.g., "gpt-4o-2024-08-06" → "gpt-4o")
    if (!pricing) {
        pricing = db.prepare('SELECT * FROM model_pricing WHERE provider = ? AND ? LIKE model || \'%\' ORDER BY length(model) DESC LIMIT 1').get(provider, model);
    }
    return pricing;
};
